# Intro Request — Ask for Warm Introduction

**Use for**: Asking a relationship to introduce you to a target investor
**Tone**: Concise, make it easy for the introducer, forwardable blurb included

---

## Template A — To Board Member / Existing Investor

**Subject**: Intro request — [Target Firm]

Hey [First],

Could you intro me to [Name] at [Firm]? They're [one line on why they fit — e.g., "running the composting fund at Closed Loop, perfect thesis match"].

I'd use the blurb below if you want to just forward it:

---

*Forwarding on behalf of Lee Robinson, CEO of Dyrt Labs — they're the ERP of Modern Waste. $1M ARR, 450% YoY, 0% churn. Customers include Disney, Marriott, Intuit Dome, AEG, Sprouts. Raising $10M Series A. The [waste / circular / climate] thesis maps directly. Lee is great — worth a quick call.*

*Lee Robinson | lee@dyrt.co | 813.390.8559*

---

Thanks,

Lee

---

## Template B — To Peer Founder / Operator

**Subject**: Quick favor — investor intro?

Hey [First],

Do you have a relationship with [Name] at [Firm]? We're raising our Series A — they'd be a great fit ([one line reason]).

Happy to do the same for you anytime.

[Forwardable blurb if helpful:]

---

*Lee Robinson, CEO of Dyrt Labs — ERP of Modern Waste. $1M ARR, 450% YoY, 0% churn. 45 enterprise customers: Disney, Marriott, AEG, Intuit Dome. Raising $10M Series A. Clean thesis match for [firm].*

---

Thanks,
Lee

---

## Template C — Via LinkedIn (When No Email Connection)

**Use for**: LinkedIn connection + intro request
**Tone**: Ultra brief

Hi [Name] — I'm the CEO of Dyrt Labs, the ERP of Modern Waste. $1M ARR, 450% YoY, 0% churn — customers include Disney, Marriott, AEG. We're raising a $10M Series A and [firm]'s [thesis] is a direct fit. Would love 20 minutes.

Lee Robinson | lee@dyrt.co | dyrt.co
